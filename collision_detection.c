...
static int setupCamera(const char *cparam_name, char *vconf, ARParamLT **cparamLT_p, ARHandle **arhandle,
        ...
        case 'x':
            ...
            switch (mode) {
                case AR_IMAGE_PROC_FRAME_IMAGE:
                    mode = AR_IMAGE_PROC_FIELD_IMAGE;
                    break;
                case AR_IMAGE_PROC_FIELD_IMAGE:
                default:
                    mode = AR_IMAGE_PROC_FRAME_IMAGE;
                    break;
            }
        ...
        case 'A':
            ...
            switch (modea) {
                case AR_LABELING_THRESH_MODE_MANUAL:
                    modea = AR_LABELING_THRESH_MODE_AUTO_MEDIAN;
                    break;
                case AR_LABELING_THRESH_MODE_AUTO_MEDIAN:
                    modea = AR_LABELING_THRESH_MODE_AUTO_OTSU;
                    break;
                case AR_LABELING_THRESH_MODE_AUTO_OTSU:
                    modea = AR_LABELING_THRESH_MODE_AUTO_ADAPTIVE;
                    break;
                ...
                default:
                    modea = AR_LABELING_THRESH_MODE_MANUAL;
                    break;
            }
        case 'e':
        case 'E':
...
static void mainLoop(void) {
    ...
    if ((image = arVideoGetImage()) != NULL) {
        ...
        if (gARTImageSavePlease) {
            ...
            if (arVideoSaveImageJPEG(gARHandle->xsize, gARHandle->ysize, gARHandle->arPixelFormat, gARTImage,
...
static void drawBackground(const float width, const float height, const float x, const float y) {
    ...
    vertices[0][0] = x;
    vertices[0][1] = y;
    vertices[1][0] = width + x;
    vertices[1][1] = y;
    vertices[2][0] = width + x;
    vertices[2][1] = height + y;
    vertices[3][0] = x;
    vertices[3][1] = height + y;
    glLoadIdentity();
...
static void printMode() {
    ...
    switch (threshMode) {
        case AR_LABELING_THRESH_MODE_MANUAL:
            ...
            break;
        case AR_LABELING_THRESH_MODE_AUTO_MEDIAN:
            text_p = (char *) "AUTO_MEDIAN";
            break;
        case AR_LABELING_THRESH_MODE_AUTO_OTSU:
            text_p = (char *) "AUTO_OTSU";
            break;
        case AR_LABELING_THRESH_MODE_AUTO_ADAPTIVE:
            text_p = (char *) "AUTO_ADAPTIVE";
            break;
        case AR_LABELING_THRESH_MODE_AUTO_BRACKETING:
            text_p = (char *) "AUTO_BRACKETING";
            break;
        default:
            text_p = (char *) "UNKNOWN";
            break;
    ...
    switch (mode) {
        case AR_TEMPLATE_MATCHING_COLOR:
            text_p = (char *) "Colour template (pattern)";
            break;
        case AR_TEMPLATE_MATCHING_MONO:
            text_p = (char *) "Mono template (pattern)";
            break;
        case AR_MATRIX_CODE_DETECTION:
            text_p = (char *) "Matrix (barcode)";
            break;
        case AR_TEMPLATE_MATCHING_COLOR_AND_MATRIX:
            text_p = (char *) "Colour template + Matrix (2 pass, pattern + barcode)";
            break;
        case AR_TEMPLATE_MATCHING_MONO_AND_MATRIX:
            text_p = (char *) "Mono template + Matrix (2 pass, pattern + barcode ";
            break;
        default:
            text_p = (char *) "UNKNOWN";
            break;
